	...
	<div class="row">
			...
			<fieldset class="cuerpo">
				<fieldset >
					...
					<legend class="cuerpo"><h4>Solicitud de talleres</h4></legend>
					<div class="twelve columns">
					...
					<form action='<?=base_url();?>cursos_c/generaOrdenPago' method='post' name='process' accept-charset="utf-8" class="custom">
						...
						<a class="button" href='<?=base_url();?>cursos_c/listaCursos/<?=$idUsuario?>/<?=$tipo?>'>Regresar</a>
						<?php if(isset($i) && $i != 0): ?>
							<input type="submit" id="sigteBtn" class="button" style="float: right;" value="Confirmar solicitud" />
						<?php endif;?>
						<?php if(isset($i) && $i == 0): ?>
							<input type="submit" id="sigteBtn" class="button" style="float: right;" value="Confirmar solicitud" disabled/>
						<?php endif;?>
						</div>
