<html>
<head>
	<title>Lista de cursos talleres</title>
	<meta charset="utf-8" />
	<meta charset="utf-8" />
	<meta name="viewport" content="width=device-width" />
  	...
  	<!-- Si pido almenos un cambio la variable para que solo pueda seleccionar un curso (js.js) -->
  	<?php if(isset($numCursos) && ($numCursos > 0)){ ?>
  		<script type="text/javascript">
  			var maxi=1;
  		</script>
  	<?php } ?>
					      ...
					      <?php if(isset($cursos) && !empty($cursos)){
					      	foreach ($cursos as $key => $value) {
					      		if(isset($productos[$value['IdProducto']])){
									...
									echo '<label><input type="checkbox" onclick="validar('."IdProducto".$value['IdProducto'].',0)" name='."IdProducto".$value['IdProducto'].' value='.$value['IdProducto'].' >'.$value['Producto'].' <b>Horario: '.$value['Descripcion'].'</b></label>';
								}
							}
					      }?>
					      <?php if(isset($cursos) && empty($cursos)): ?>
