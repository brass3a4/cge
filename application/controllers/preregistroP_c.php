<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class PreregistroP_c extends CI_Controller{
		...
		function preDatos()
		{
			...
			$datos = $_POST;
			$datos['Usuarios_Nombre'] = ucwords(mb_strtolower($datos['Usuarios_Nombre']));
			$datos['Usuarios_aPaterno'] = ucwords(mb_strtolower($datos['Usuarios_aPaterno']));
			$datos['Usuarios_aMaterno'] = ucwords(mb_strtolower($datos['Usuarios_aMaterno']));
			// echo "<pre>";
			// 	print_r($datos);
			// echo "</pre>";
			...
			$msj = '<html>
						...
						<body>
							...
							Estimado candidato, <b>'.$data['Usuarios_Nombre'].' '.$data['Usuarios_aPaterno'].' '.$data['Usuarios_aMaterno'].', </b> 
											...
											<a href="'.base_url().'login_c">aquí.</a> En este paso debe subir todos los documentos solicitados en la convocatoria. <br>
											Nota: Le solicitamos guardar este correo electrónico para poder tener a la mano, en el momento que lo requiera, sus datos de acceso. <br>
							</tr>
							<tr>
