...
function rbl_elt( _id, _type ) { 
	...
	this.drawAfterElt = function(isHt) { 
		...
		if ( this.e_help_text && this.e_help_text.length > 0 && this.e_type != "wizard" && this.e_type != "label" && this.e_type != "image" && this.e_type != "button") {
			str = "<img class='imghelp'  src='img/rbl/help-browser.png' border='0' title='" + this.e_help_text.replace(reg,"`") +"' alt='" + this.e_help_text.replace(reg,"`")+ "' width='16' height='16' hspace='10'>" ;
		}
	...
	this.drawDate = function(isHt) { 
		...
		str = "<input type='text' " + this.drawName() 
									+ this.drawAutoskip() 
									+ this.drawReadonly() 
									+ this.drawRequired() 
									+ this.drawSize() 
		...
		if (isHt) {
			...
			str += "<img src='img/rbl/_cal.gif' border='0' hspace='2' onclick=\"displayCalendarName('" + this.e_id + "','" + z + "',this);return false;\" >" ;
		}
  ...
  this.drawLabel = function(isHt) { 
	...
	this.drawXml = function(_col, _ligne) { 
		...
		str = "<element type_html='" + this.e_type +"'" 
...
function supElt( _no ) {
		...
		jQuery("#f_" + _no).fadeOut(500, function() { jQuery(this).remove(); });
...
function loadElement( elt, ncol, nbel) {
	...
	rblEltArray[ _id ].e_type  = ( ( jQuery(elt).attr('type_html') ) ? jQuery(elt).attr('type_html').toLowerCase() : "text" ) ;
	rblEltArray[ _id ].e_name 	=  ( ( jQuery(elt).attr('name') ) ? jQuery(elt).attr('name') : _id ) ;
	rblEltArray[ _id ].e_label 	=  ( ( jQuery(elt).attr('label') ) ? jQuery(elt).attr('label') : "" ) ;
	rblEltArray[ _id ].e_size	=  ( ( jQuery(elt).attr('size') ) ? jQuery(elt).attr('size') : "" ) ;

	rblEltArray[ _id ].e_typeint =  ( ( jQuery(elt).attr('type_interne') ) ? jQuery(elt).attr('type_interne') : "" ) ; 
		
	rblEltArray[ _id ].e_mask	 =   ( ( jQuery(elt).attr('mask') ) ? jQuery(elt).attr('mask') : "" ) ;  
	rblEltArray[ _id ].e_freemask =  ( ( jQuery(elt).attr('freemask') ) ? jQuery(elt).attr('freemask') : "" ) ;  
	rblEltArray[ _id ].e_regexp	  =  ( ( jQuery(elt).attr('regexp') ) ? jQuery(elt).attr('regexp') : "" ) ;  
	rblEltArray[ _id ].e_maxlength = ( ( jQuery(elt).attr('maxlength') ) ? jQuery(elt).attr('maxlength') : ( ( jQuery(elt).attr('max_length') ) ? jQuery(elt).attr('max_length') : "" ) ) ;  
	rblEltArray[ _id ].e_accesskey = ( ( jQuery(elt).attr('accesskey') ) ? jQuery(elt).attr('accesskey') : "" ) ;  
	rblEltArray[ _id ].e_val_liste = ( ( jQuery(elt).attr('val_liste') ) ? jQuery(elt).attr('val_liste') : "" ) ;  
		
	rblEltArray[ _id ].e_val_defaut =  ( ( jQuery(elt).attr('val_defaut') ) ? jQuery(elt).attr('val_defaut') : "" ) ;  
	rblEltArray[ _id ].e_css_class = ( ( jQuery(elt).attr('class') ) ? jQuery(elt).attr('class') : "" ) ;  
	...
	zz = ( ( jQuery(elt).attr('required') ) ? jQuery(elt).attr('required') : jQuery(elt).attr('obligatoire') ) ;  
	rblEltArray[ _id ].e_required = ( zz == 'O' || zz == 'Y' ) ;

	zz = jQuery(elt).attr('readonly') ;
	rblEltArray[ _id ].e_readonly = ( zz == 'O' || zz == 'Y' ) ;

	jQuery('#sortable' + ncol).append("<li id='f_" + _id +"' class='elt'>" + 
