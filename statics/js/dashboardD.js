$(document).on('ready',function(){
	...
	llenarTabla(2,4,'extranjeros','modalDivsExtra','cuerpoExtranjero');
	llenarTabla(1,4,'nacional','modalDivsNacio','cuerpoNacional');
	llenarTablaAceptados(4,'aceptados','modalDivsAcept','cuerpoAceptados');
	llenarTablaSinInter(4,'sinInteraccion','modalDivsSinInter','cuerpoSinInter');
	...
	function llenarTabla(estudios,rol,idTabla,idDivModal,idTBody){
		...
		rol: rol
			...
			$.each(usuarios, function(key,value){
				...
				$('#archivos'+value.IdUsuario).append("<a href='' title='Constancia (Vuelta)' data-reveal-id='constAtras"+value.IdUsuario+"'><div onclick=\"reset('"+value.constAtras.IdArchivo+"', 'http://localhost/cge/dashboard_c/"+value.constAtras.IdArchivo+"', 'Archivo "+value.constAtras.IdArchivo+"', 'es','"+value.constAtras.IdArchivo+"');\" class='large-2 small-2 columns circulo "+value.constAtras.color+" left'><p>C</p></div></a>");
				$('#archivos'+value.IdUsuario).append("<a href='' title='Constancia (Frente)' data-reveal-id='constFrente"+value.IdUsuario+"'><div onclick=\"reset('"+value.constFrente.IdArchivo+"', 'http://localhost/cge/dashboard_c/"+value.constFrente.IdArchivo+"', 'Archivo "+value.constFrente.IdArchivo+"', 'es','"+value.constFrente.IdArchivo+"');\" class='large-2 small-2 columns circulo "+value.constFrente.color+" left'><p>C</p></div></a>");
				...
				$('#'+idDivModal).append('<div id="constAtras'+value.IdUsuario+'" class="reveal-modal" data-reveal><img id="constAtrasImg'+value.IdUsuario+'" src="" alt=""><a class="close-reveal-modal">&#215;</a></div>');
				$('#'+idDivModal).append('<div id="constFrente'+value.IdUsuario+'" class="reveal-modal" data-reveal><img id="constFrenteImg'+value.IdUsuario+'" src="" alt=""><a class="close-reveal-modal">&#215;</a></div>');
				...
				$('#constAtrasImg'+value.IdUsuario).attr('src',urlBase+value.constAtras.url);
				$('#constAtrasImg'+value.IdUsuario).addClass(value.constAtras.color+"Out");
				$('#constAtrasImg'+value.IdUsuario).addClass("large-12");

				$('#constFrenteImg'+value.IdUsuario).attr('src',urlBase+value.constFrente.url);
				$('#constFrenteImg'+value.IdUsuario).addClass(value.constFrente.color+"Out");
				$('#constFrenteImg'+value.IdUsuario).addClass("large-12");
				...
				if((value.constAtras.color) != 'vacia'){
					$('#constAtrasImg'+value.IdUsuario).before('<div id="menu'+value.constAtras.IdArchivo+'" class="large-12 row smnall-12 columns pbtm"></div>');
					$('#menu'+value.constAtras.IdArchivo).append("<div class='row'><div class='large-12 small-12 pbtm'><h5>Constancia de estudios (Vuelta)</h5></div></div>");
					$('#menu'+value.constAtras.IdArchivo).append("<div id='acepta"+value.constAtras.IdArchivo+"' class='large-3 small-3 columns btn btn-aceptar'>Aceptar</div>");
					$('#menu'+value.constAtras.IdArchivo).append("<a class='large-3 small-3 columns btn btn-print' href='"+urlBase+"dashboard_c/documentoPdf/"+value.constAtras.IdArchivo+"' target='_blank' class='btn btn-print'>Descargar</a>");
					$('#menu'+value.constAtras.IdArchivo).append("<div id='rechaza"+value.constAtras.IdArchivo+"' class='large-3 small-3 columns btn btn-rechazar'>Rechazar</div>");
					$('#constAtrasImg'+value.IdUsuario).after('<div id="disqus'+value.constAtras.IdArchivo+'" class="large-12 smnall-12 columns ptop"></div>');
				}
				
				$('#acepta'+value.constAtras.IdArchivo).on('click',function(){
					var idArchivo = value.constAtras.IdArchivo;
				...
				$('#rechaza'+value.constAtras.IdArchivo).on('click',function(){
					var idArchivo = value.constAtras.IdArchivo;
				...
				if((value.constFrente.color) != 'vacia'){
					$('#constFrenteImg'+value.IdUsuario).before('<div id="menu'+value.constFrente.IdArchivo+'" class="large-12 smnall-12 columns pbtm"></div>');
					$('#menu'+value.constFrente.IdArchivo).append("<div class='row'><div class='large-12 small-12 pbtm'><h5>Constancia de estudios (Frente)</h5></div></div>");
					$('#menu'+value.constFrente.IdArchivo).append("<div id='acepta"+value.constFrente.IdArchivo+"' class='large-2 small-2 columns btn btn-aceptar left'>Aceptar</div>");
					$('#menu'+value.constFrente.IdArchivo).append("<a class='large-3 small-3 columns btn btn-print' href='"+urlBase+"dashboard_c/documentoPdf/"+value.constFrente.IdArchivo+"' target='_blank' >Descargar</a>");
					$('#menu'+value.constFrente.IdArchivo).append("<div id='rechaza"+value.constFrente.IdArchivo+"' class='large-2 small-2 columns btn btn-rechazar right'>Rechazar</div>");
					$('#constFrenteImg'+value.IdUsuario).after('<div id="disqus'+value.constFrente.IdArchivo+'" class="large-12 smnall-12 columns ptop"></div>');
				}

				$('#acepta'+value.constFrente.IdArchivo).on('click',function(){
					var idArchivo = value.constFrente.IdArchivo;
					$.post(urlBase+"index.php/dashboard_c/aceptarDocumento", {
						idArchivo:idArchivo
				    }, function(data) {
				    	console.log(data);
				...
				$('#rechaza'+value.constFrente.IdArchivo).on('click',function(){
					var idArchivo = value.constFrente.IdArchivo;
					$.post(urlBase+"index.php/dashboard_c/rechazarDocumento", {
						idArchivo:idArchivo
				    }, function(data) {
				...
				var nombreArchivos = new Array('constAtras','constFrente','cv','ifeAtras','ifeFrente','tituloLicAtras','tituloLicFrente');
